...
type EmbeddingSetting struct {
	...
	Dimensions      int32  `gorm:"column:dimensions;not null;check:embedding_settings_dimensions_check,dimensions > 0" json:"dimensions"`
	TimeoutSeconds  int32  `gorm:"column:timeout_seconds;not null;check:embedding_settings_timeout_seconds_check,timeout_seconds > 0" json:"timeout_seconds"`
	APIKeyEncrypted string `gorm:"column:api_key_encrypted;type:text;not null;default:''" json:"-"`
	IsActive        bool   `gorm:"column:is_active;not null;default:false;uniqueIndex:embedding_settings_one_active_idx,where:is_active" json:"is_active"`
}
