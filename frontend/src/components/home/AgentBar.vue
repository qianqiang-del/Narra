...
import { PRESET_ROLES, TEACHER, type AgentRole, type AgentBarMode } from '@/data/agents'
...
const teacherVoice = defineModel<string>('teacherVoice', { default: TEACHER.voice })

const rootRef = ref<HTMLElement | null>(null)
const open = ref(false)

const roleVoices = ref<Record<string, string>>(
  Object.fromEntries(PRESET_ROLES.map((r: AgentRole) => [r.id, r.voice])),
)

const selectedRoles = computed(() => PRESET_ROLES.filter((r) => selectedIds.value.includes(r.id)))
...
const autoPreviewAvatars = computed(() => PRESET_ROLES.slice(0, 3))
      ...
      >
        ...
        <div class="flex shrink-0 items-center gap-1.5">
          ...
          <div class="size-8 overflow-hidden rounded-full ring-2 ring-blue-400/40">
            <img :src="TEACHER.avatar" alt="" class="size-full object-cover" />
          </div>
    ...
    >
      ...
      >
        ...
        <div class="mb-2 flex items-center gap-2 rounded-lg bg-primary/5 px-2.5 py-1.5">
          ...
          <span class="text-[13px] font-medium">{{ TEACHER.name }}</span>
          <div class="ml-auto">
        ...
        <div v-if="mode === 'preset'" class="max-h-56 overflow-y-auto">
          ...
          >
            ...
            <div
              ...
              :style="selectedIds.includes(r.id) ? { boxShadow: `0 0 0 2px ${r.color}30` } : undefined"
            >
              <img :src="r.avatar" alt="" class="size-full object-cover" />
            </div>
