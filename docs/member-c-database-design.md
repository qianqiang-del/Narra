...
- `classrooms`：课程主记录。
- `scenes`：课程中的课件页。
- `preset_agents`：全局预设角色资料库。
- `classroom_agents`：某门课程实际选择的角色和课程音色。

Agent 身份统一使用 `classroom_agents.id`。需要角色名称、人设、类型、头像或主题色时，通过
`classroom_agents.agent_id -> preset_agents.id` 查询。

由于 `classroom_agents` 只保存当前角色集合，历史消息和回合必须另外保存角色快照。否则课程更换
角色后，旧消息会丢失显示身份。
